...
# ================= 🔔 REMINDER FUNCTION =================
def check_medicine_reminders(user_id):
    now = datetime.now().strftime("%H:%M")
    today = date.today().strftime("%Y-%m-%d")
    meds = get_medicines_for_date(user_id, today)
...
# ================= HOME =========================
    ...
    if today_meds:
        ...
        # 🔔 CHECK REMINDER (NO UI CHANGE)
    check_medicine_reminders(user[0])

    today_date = date.today().strftime("%Y-%m-%d")
    meds = get_medicines_for_date(user[0], today_date)
        ...
        for t in med["times"]:
            ...
            with col2:
                if taken and taken[0]:
                    ...
                    if st.button("Take", key=f"{med['id']}_{t}"):
                        mark_as_taken(med["id"], today_date, t)
                        st.rerun()
...
# ================= CALENDAR =================
    ...
    st.markdown("### View your monthly medicine intake history")
    st.markdown("---")
    ...
    if st.session_state.get('view_day_details', False):
        ...
        day_meds = get_medicines_for_date(user[0], selected_date)
