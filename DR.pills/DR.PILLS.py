...
# ================= MAIN APP =================
    ...
    def get_mascot_state():
        ...
        # Daytime - Check mascot based on number of medicines taken
        if total_scheduled == 0:
            return None
        
        # Handle missed medicines first
        if missed_count > 0:
            return {
                'emotion': 'sad',
        ...
        # Handle upcoming medicines (time to take now)
        if upcoming_count > 0:
            return {
                'emotion': 'urgent',
        ...
        # NEW LOGIC: Based on number of medicines taken
        if taken_count == 0:
            # No medicines taken - Sad
            return {
                'emotion': 'sad',
                'message': f"You haven't taken any medicine yet. Please take your medicines to stay healthy! 😢",
                'missed_list': None
            }
        elif taken_count < total_scheduled:
            # Some medicines taken (1 or 2) but not all - Urgent/Angry
            remaining = total_scheduled - taken_count
            return {
                'emotion': 'urgent',
                'message': f"You've taken {taken_count} medicine{'s' if taken_count != 1 else ''} but still have {remaining} left! Keep going! 😠",
                'missed_list': None
            }
        elif taken_count == total_scheduled and total_scheduled > 0:
            # All medicines taken - Happy
            return {
                'emotion': 'happy',
...
def mark_as_taken(medicine_id, target_date, time_slot):
    cursor.execute(
        "SELECT id FROM tracking WHERE medicine_id=? AND date=? AND time_slot=?",
4            "INSERT INTO tracking (medicine_id, date, time_slot, taken, timestamp) VALUES (?, ?, ?, ?, ?)",
            (medicine_id, target_date, time_slot, True, datetime.now())
        )
    conn.commit()
...
def calculate_weekly_adherence(user_id):
    ...
    for i in range(7):
        ...
        day_name = (d - timedelta(days=6-i)).strftime('%a')
        adherence = calculate_adherence(user_id, target_date)
4    # Get adherence for last 7 days
    last_7_days = []
    for i in range(7):
        d = date.today()
        target_date = (d - timedelta(days=i)).strftime("%Y-%m-%d")
...
12.28 7.1#import streamlit as st
import sqlite3
import pandas as pd
...
def get_weekly_adherence_chart_data(user_id):
    data = []
    today = date.today()
    ...
    for i in range(6, -1, -1):
        ...
        data.append({
            "Day": d.strftime("%a"),
            "Adherence (%)": calculate_adherence(user_id, d.strftime("%Y-%m-%d"))
        })
...
if st.session_state.user is None and st.session_state.auth_mode is None:
 ...
 ================= HOME =========================
    ...
    weekly_df = get_weekly_adherence_chart_data(user[0])
